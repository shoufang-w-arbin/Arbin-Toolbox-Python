...
class GetBarcodeInfoFeedback(DictReprBase):
    def __init__(self, obj):
        if not isinstance(obj, ArbinDataModel.RequestInformation.GetBarcodeInfoFDBK):
            raise ValueError("'obj' must be of type ArbinDataModel.RequestInformation.GetBarcodeInfoFDBK")
...
class SubscribeTestInfoDataFeedback(DictReprBase):
    class TestInfoData(DictReprBase):
        ...
        def __init__(self, data: ArbinDataModel.TestInfoData):
            ...
            self.eqs                     = [self.TestInfoSPTTEQ(x) for x in data.EQs]
            self.cells                   = [self.TestInfoSPTTCell(x) for x in data.Cells]
