...
class TestFeedbackClasses(unittest.TestCase):
    ...
    def test_GetStartDataFeedback_instantiation(self):
        ...
        self.assertEqual(len(feedback_instance.m_channels), 1)
        channel = feedback_instance.m_channels[0]
    ...
    def test_GetChannelDataFeedback_instantiation(self):
        ...
        self.assertEqual(len(feedback_instance.m_channels), 1)
        m_channels = feedback_instance.m_channels[0]
        self.assertEqual(m_channels.channel_index, 1)
        self.assertEqual(m_channels.status, GetChannelDataFeedback.EChannelStatus.External_Charge)
        self.assertFalse(m_channels.comm_failure)
        self.assertEqual(m_channels.schedule, "TestSchedule")
        self.assertEqual(m_channels.can_cfg, "CANConfig")
        self.assertEqual(m_channels.smb_cfg, "SMBConfig")
        self.assertEqual(m_channels.chart, "TestChart")
        self.assertEqual(m_channels.test_name, "TestName")
        self.assertEqual(m_channels.exit_condition, "ExitCondition")
        self.assertEqual(m_channels.step_and_cycle, "StepCycle")
        self.assertEqual(m_channels.step_index, 2)
        self.assertEqual(m_channels.cycle_index, 3)
        self.assertEqual(m_channels.barcode, "123456789")
        self.assertEqual(m_channels.master_channel, 0)
        self.assertAlmostEqual(m_channels.test_time, 123.45, places=6)
        self.assertAlmostEqual(m_channels.step_time, 12.34, places=6)
        self.assertAlmostEqual(m_channels.voltage, 3.7, places=6)
        self.assertAlmostEqual(m_channels.current, 1.5, places=6)
        self.assertAlmostEqual(m_channels.power, 5.55, places=6)
        self.assertEqual(m_channels.test_object, "Battery")
        self.assertAlmostEqual(m_channels.nominal_capacity, 2500.0, places=6)
        self.assertAlmostEqual(m_channels.imax, 2.0, places=6)
        self.assertAlmostEqual(m_channels.vmax, 4.2, places=6)
        self.assertAlmostEqual(m_channels.vmin, 3.0, places=6)
        self.assertAlmostEqual(m_channels.nominal_voltage, 3.7, places=6)
        self.assertAlmostEqual(m_channels.nominal_capacitance, 0.0, places=6)
        self.assertAlmostEqual(m_channels.nominal_ir, 0.01, places=6)
        self.assertAlmostEqual(m_channels.specific_capacity, 1.0, places=6)
        self.assertTrue(m_channels.is_auto_calculate)
        self.assertAlmostEqual(m_channels.mass, 50.0, places=6)
        self.assertAlmostEqual(m_channels.charge_capacity, 1000.0, places=6)
        self.assertAlmostEqual(m_channels.discharge_capacity, 800.0, places=6)
        self.assertAlmostEqual(m_channels.charge_energy, 50.0, places=6)
        self.assertAlmostEqual(m_channels.discharge_energy, 40.0, places=6)
        self.assertAlmostEqual(m_channels.internal_resistance, 0.005, places=6)
        self.assertAlmostEqual(m_channels.dvdt, 0.1, places=6)
        self.assertAlmostEqual(m_channels.dvdq, 0.05, places=6)
        self.assertAlmostEqual(m_channels.dqdv, 0.02, places=6)
        self.assertAlmostEqual(m_channels.acr, 0.001, places=6)
        self.assertAlmostEqual(m_channels.aci, 0.002, places=6)
        self.assertAlmostEqual(m_channels.aci_phase, 0.5, places=6)
        self.assertEqual(len(m_channels.can), 1)
        self.assertEqual(m_channels.can[0].idx, 123)
        
        self.assertEqual(len(m_channels.smb), 1)
        self.assertEqual(m_channels.smb[0].idx, 456)

        self.assertEqual(len(m_channels.auxs), 12)
        self.assertEqual(len(m_channels.auxs[0]), 1)
        self.assertAlmostEqual(m_channels.auxs[0][0].value, 1.23, places=5)
        self.assertAlmostEqual(m_channels.auxs[0][0].value_dt, 45.6, places=5)

        self.assertEqual(len(m_channels.eq_data), 1)
        self.assertEqual(m_channels.eq_data[0].parent_tray_global_index, 34)

        self.assertEqual(len(m_channels.cell_data), 1)
        self.assertEqual(m_channels.cell_data[0].parent_tray_global_index, 56)
